      ...
      axios.post('/taikhoan/register',{
          ...
          confirmPasswd:confirmPasswd     
           ...
           if (error.response.status === 400) setError(error.response.data.message);
            ...
            else if(error.response.status === 402) setError(error.response.data.message);
              else if(error.response.status === 403) setError(error.response.data.message);
        ...
        <form >
            <h3>Đăng kí tài khoản</h3>
                 ...
                 <label>Giới tính</label>
                  <div className="form-check">
                    <div className="form-check form-check-inline">
                        <input className="form-check-input" type="radio" name="isMale" id="inlineRadio1" value="1"
                                onChange={val=> setIsMale(val.target.value)}
                        />
                        <label className="form-check-label">Nam</label>
                    </div>
                    <div className="form-check form-check-inline">
                        <input className="form-check-input" type="radio" name="isMale" id="inlineRadio2" value="0"
                                onChange={val=> setIsMale(val.target.value)}
                        />
                ...
                <div className="d-grid gap-2">
                  <button onClick={handleSubmit} className="btn btn-primary" type="button">Đăng kí</button>
